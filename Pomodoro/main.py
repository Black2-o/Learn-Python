...
def reset_timer():
    ...
    check_button_label.config(text="")
    global reps
    reps = 0
# ---------------------------- TIMER MECHANISM ------------------------------- # 
def start_timer():
...
def count_down(count):
    ...
    if count > 0:
        ...
        for _ in range(work_session):
            mark += "✔"
        check_button_label.config(text=mark)
...
str_button = Button(text="Start", highlightbackground=YELLOW, command=start_timer)
reset_button = Button(text="Reset", highlightbackground=YELLOW, command=reset_timer)
str_button.grid(column=0, row=3)
reset_button.grid(column=3, row=3)


check_button_label = Label(font=(FONT_NAME, 10, "bold"), bg=YELLOW, fg=GREEN)
check_button_label.grid(column=2, row=4)
